...
function maschan_extract_full_vendor($user_id) {
    ...
    return [
        ...
        'joined_date'       => !empty($user->user_registered) ? date('Y-m-d', strtotime($user->user_registered)) : date('Y-m-d'),
        'socials'           => $socials,
...
/**
 * Validasi input ulasan produk — SATU-SATUNYA tempat aturan ini
 * boleh berada, dipakai di endpoint POST /products/{id}/reviews.
 *
 * @param array $params  Array parameter mentah dari request (belum di-sanitize).
 * @return true|WP_Error Kembalikan true jika valid; WP_Error dengan detail
 *                        pesan bahasa Indonesia jika tidak valid.
 */
function maschan_validate_review_input(array $params) {
    // --- Nama Penulis ---------------------------------------------------
    ...
    // --- Email Penulis (opsional, tapi jika diisi harus valid) ----------
    ...
    // --- Rating ---------------------------------------------------------
    // Wajib integer 1–5; tolak nilai non-numerik atau di luar rentang.
    ...
    if ($rating < 1 || $rating > 5) {
        return new WP_Error(
            'invalid_rating',
            'Rating harus bernilai antara 1 dan 5.',
            ['status' => 400]
        );
    }
    ...
    if (mb_strlen($content) > 2000) {
        return new WP_Error(
            'content_too_long',
            'Isi ulasan maksimal 2000 karakter.',
            ['status' => 400]
        );
    }

    // --- Foto Ulasan (opsional, maks 5 URL valid) -----------------------
    if (!empty($params['images'])) {
        if (!is_array($params['images'])) {
            return new WP_Error(
                'invalid_images_format',
                'Field images harus berupa array URL gambar.',
                ['status' => 400]
            );
        }
        if (count($params['images']) > 5) {
            return new WP_Error(
                'too_many_images',
                'Maksimal 5 foto yang dapat dilampirkan dalam satu ulasan.',
                ['status' => 400]
            );
        }
        foreach ($params['images'] as $img_url) {
            if (!filter_var($img_url, FILTER_VALIDATE_URL)) {
                return new WP_Error(
                    'invalid_image_url',
                    'Setiap foto harus berupa URL yang valid.',
                    ['status' => 400]
                );
            }
        }
    ...
    // --- URL Video (opsional, harus URL valid jika diisi) ---------------
    if (!empty($params['video_url'])) {
        if (!filter_var(trim($params['video_url']), FILTER_VALIDATE_URL)) {
            return new WP_Error(
                'invalid_video_url',
                'URL video tidak valid. Gunakan tautan YouTube, TikTok, atau Google Drive yang benar.',
                ['status' => 400]
            );
        }
    }
...
add_action('rest_api_init', function () {
        ...
        'callback' => function ($request) {
            ...
            return rest_ensure_response([
                ...
                'user'    => [
                    ...
                    'role'        => user_can($user->ID, 'manage_options') ? 'admin' : 'vendor',
                ]
            ]);
        },
    ...
    // ENDPOINTS REVIEW / ULASAN PRODUK
    register_rest_route('maschan/v1', '/products/(?P<id>\d+)/reviews', [
        [
            'methods'  => 'GET',
            'callback' => function ($request) {
                $product_id = (int)$request['id'];
                ...
                if (!empty($comments)) {
                    foreach ($comments as $comment) {
                        ...
                        $rating = !empty($rating_meta) ? (int)$rating_meta : 5;
                        $total_rating += $rating;

                        $raw_images = get_comment_meta($comment->comment_ID, 'review_images', true);
                        $images_list = !empty($raw_images) ? json_decode($raw_images, true) : [];
        ...
        [
            ...
            'callback' => function ($request) {
                ...
                // Validasi terpusat — delegasikan ke fungsi Single Source of Truth
                $validation = maschan_validate_review_input($params);
                ...
                if (!$comment_id) {
                    return new WP_Error('insert_failed', 'Gagal menyimpan ulasan.', ['status' => 500]);
                }
                ...
                if (!empty($params['images']) && is_array($params['images'])) {
                    $saved_images = array_slice(array_map('esc_url_raw', $params['images']), 0, 5);
                    update_comment_meta($comment_id, 'review_images', wp_json_encode($saved_images));
                }

                // Simpan link video ulasan
                $saved_video = '';
                if (!empty($params['video_url'])) {
                    $saved_video = esc_url_raw(trim((string)$params['video_url']));
                    update_comment_meta($comment_id, 'review_video_url', $saved_video);
                }
    ...
    // ENDPOINTS ADMIN MODERASI ULASAN PRODUK
    register_rest_route('maschan/v1', '/admin/reviews', [
        'methods'  => 'GET',
        'callback' => function ($request) {
            $admin_id = maschan_get_authenticated_admin_id($request);
            if (!$admin_id) {
            ...
            $status_param = sanitize_text_field($request->get_param('status') ?: 'pending');
            $wp_status = 'hold';
            ...
            // Hitung ulasan berstatus pending secara independen
            $pending_count = (int) get_comments([
                'type'   => 'review',
                'status' => 'hold',
                'count'  => true,
            ]);
            ...
            if (!empty($comments)) {
                foreach ($comments as $comment) {
                    ...
                    $product_name = $product ? $product->post_title : 'Produk';
                    $product_slug = $product ? $product->post_name : '';
                    $vendor_id = $product ? (int)$product->post_author : 0;
                    $vendor_name = 'Mas Chan Digital';
                    if ($vendor_id) {
                        $v = maschan_extract_full_vendor($vendor_id);
                        if ($v && !empty($v['store_name'])) {
                    ...
                    $reviews[] = [
                        ...
                        'content'      => $comment->comment_content,
                        'status'       => $status,
                        'created_at'   => date('c', strtotime($comment->comment_date)),
                        'images'       => is_array($images_list) ? $images_list : [],
                        'video_url'    => (string) $video_link,
                    ];
                }
    ...
    register_rest_route('maschan/v1', '/admin/reviews/(?P<id>\d+)/action', [
        'methods'  => 'POST',
        'callback' => function ($request) {
            $admin_id = maschan_get_authenticated_admin_id($request);
            if (!$admin_id) {
            ...
            if (!$comment) {
                return new WP_Error('invalid_review', 'Ulasan tidak ditemukan.', ['status' => 404]);
            }
            ...
            if ($action === 'approve') {
                ...
                if (!empty($reason)) {
                    update_comment_meta($comment_id, 'rejection_reason', $reason);
                }
                return rest_ensure_response([
                    'success' => true,
                ...
                $deleted = wp_delete_comment($comment_id, true); // true = force delete permanen dari database
                if ($deleted) {
                    return rest_ensure_response([
                        'success' => true,
            ...
            return new WP_Error('invalid_action', "Aksi harus bernilai 'approve', 'reject', atau 'delete'.", ['status' => 400]);
        },
        'permission_callback' => '__return_true',
    ]);
